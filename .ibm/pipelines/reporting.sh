...
get_artifacts_url() {
  ...
  if [ -z "${namespace}" ]; then
    log::warn "namespace parameter is empty (this is expected only for top level artifacts directory)"
  fi
...
save_data_router_junit_results() {
  ...
  log::success "JUnit results for ${namespace} adapted to Data Router format and saved to ARTIFACT_DIR and copied to SHARED_DIR"
  log::debug "Shared directory contents:"
  ls -la "${SHARED_DIR}"
}
